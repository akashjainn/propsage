...
import { devig, calculateConsensus, buildProbabilityCurve, solveFairMarketLine, calculateEdges } from '../fml/index.js';
import { predictPlayerOutcome, calculateProbabilityOver } from '../models/distributions.js';
/**
...
export class FMLService {
    ...
    async processMarket(market) {
        try {
            ...
            const edges = calculateEdges(market.books.map(b => ({
                book: b.book,
                line: b.line,
     ...
     */
    async buildModelCurve(market) {
        // Get predictive distribution for this player/market
        const distribution = predictPlayerOutcome(market.player_id, market.market, market.features, market.sport);
     ...
     */
    blendCurves(marketCurve, modelCurve, market) {
     ...
     */
    async processMarkets(markets) {
        const results = await Promise.allSettled(markets.map(market => this.processMarket(market)));
        return results
     ...
     */
    getBestEdges(markets, minEdge = 0.03) {
