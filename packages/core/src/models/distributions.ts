...
export interface PlayerFeatures {
  ...
  targets_per_game?: number  // NFL
  air_yards_share?: number   // NFL
  
  // Opponent
  opponent_dvoa?: number     // NFL defense
  opponent_drtg?: number     // NBA defense
  opponent_k_rate?: number   // MLB strikeout rate
  pace?: number              // Team pace factor
  
  // Context
  spread?: number
  total?: number
  implied_team_total?: number
  weather_impact?: number    // 0-1 scale
  rest_days?: number
  home_advantage?: boolean
  
  // Availability/Health
  injury_probability?: number  // 0-1 scale
  minutes_restriction?: number // 0-1 scale
...
export interface DistributionParams {
  ...
  params: {
    mu?: number      // mean (normal, lognormal)
    sigma?: number   // std dev (normal, lognormal)
    lambda?: number  // rate (poisson)
    r?: number       // failures (negbinom)
    p?: number       // success prob (negbinom)
  }
  sport: string
  market: string
  player_id: string
 ...
 */
export function predictPlayerOutcome(
  playerId: string,
  market: string,
 ...
 */
function predictNBAOutcome(playerId: string, market: string, features: PlayerFeatures): DistributionParams {
 ...
 */
function predictNFLOutcome(playerId: string, market: string, features: PlayerFeatures): DistributionParams {
 ...
 */
function predictMLBOutcome(playerId: string, market: string, features: PlayerFeatures): DistributionParams {
 ...
 */
export function calculateProbabilityOver(line: number, distribution: DistributionParams): number {
 ...
 */
function negativeBinomialCDF(k: number, r: number, p: number): number {
  if (k < 0) return 0
 ...
 */
function getNBABaselines(playerId: string, market: string): { mean: number, std: number } {
 ...
 */
function getNFLBaselines(playerId: string, market: string): { mean: number, std: number } {
  const baselines: Record<string, { mean: number, std: number }> = {
 ...
 */
function getMLBBaselines(playerId: string, market: string): { mean: number, std: number } {
  const baselines: Record<string, { mean: number, std: number }> = {
    'H': { mean: 1.2, std: 0.8 },
    'K': { mean: 6.5, std: 2.8 },
