"use client";
import React from 'react';
import { motion } from 'framer-motion';
import { HoverCard, AnimatedPercentage } from '@/components/ui/motion';

export interface PropCardItem {
  id: string;
  playerName: string;
  team: string;
  stat: string;
  marketLine: number;
  fairLine?: number | null;
  confidence?: number | null; // 0..1
  book?: string;
}

export function computeEdgePct(marketLine: number, fairLine?: number | null): number | null {
  if (fairLine == null || Number.isNaN(fairLine) || Number.isNaN(marketLine)) return null;
  if (marketLine === 0) return null;
  // simple relative difference as placeholder until model-provided edge is available
  return ((fairLine - marketLine) / Math.abs(marketLine)) * 100;
}

export default function PropCard({ item, onClick }: { item: PropCardItem; onClick?: () => void }) {
  const edgePct = computeEdgePct(item.marketLine, item.fairLine);
  const positive = (edgePct ?? 0) >= 0;

  return (
    <HoverCard elevated onClick={onClick} className="p-6 cursor-pointer group overflow-hidden relative" data-testid="prop-row">
      {/* Subtle gradient background */}
      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

      <div className="relative z-10">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex-1 min-w-0">
            <motion.div 
              className="font-bold text-white text-lg mb-1 truncate"
              whileHover={{ x: 2 }}
              transition={{ type: "spring", stiffness: 400, damping: 25 }}
            >
              {item.playerName}
            </motion.div>
            <div className="text-sm text-white/60 flex items-center gap-2">
              <span className="font-medium">{item.team}</span>
              <span className="w-1 h-1 rounded-full bg-white/30" />
              <span>{item.stat}{item.book ? ` · ${item.book}` : ''}</span>
            </div>
          </div>

          <div className="text-right flex flex-col items-end gap-1">
            {edgePct != null ? (
              <AnimatedPercentage value={edgePct} className="text-xl" />
            ) : (
              <div className="text-white/50 text-sm">—</div>
            )}
            {item.confidence != null && (
              <motion.div 
                className="text-[11px] text-white/50 font-medium px-2 py-1 rounded-md glass-subtle"
                whileHover={{ scale: 1.05 }}
                transition={{ type: "spring", stiffness: 400, damping: 25 }}
              >
                {Math.round((item.confidence ?? 0) * 100)}% conf
              </motion.div>
            )}
          </div>
        </div>

        {/* Prop lines visual */}
        <div className="grid grid-cols-3 gap-3">
          <motion.div 
            className="rounded-xl border border-white/15 px-3 py-2 text-white/70 glass-subtle text-center"
            whileHover={{ 
              backgroundColor: "rgba(255, 255, 255, 0.08)",
              borderColor: "rgba(255, 255, 255, 0.2)"
            }}
            transition={{ duration: 0.2 }}
          >
            <div className="text-[10px] font-medium text-white/40 mb-1">Market</div>
            <div className="font-bold tabular-nums">{item.marketLine}</div>
          </motion.div>
          
          <motion.div 
            className="rounded-xl border border-white/15 px-3 py-2 text-white/70 glass-subtle text-center"
            whileHover={{ 
              backgroundColor: "rgba(255, 255, 255, 0.08)",
              borderColor: "rgba(255, 255, 255, 0.2)"
            }}
            transition={{ duration: 0.2 }}
          >
            <div className="text-[10px] font-medium text-white/40 mb-1">Fair</div>
            <div className="font-bold tabular-nums">{item.fairLine ?? '—'}</div>
          </motion.div>
          
          <motion.div 
            className={`rounded-xl border px-3 py-2 glass-subtle text-center font-bold ${
              (edgePct ?? 0) === 0
                ? 'border-white/15 text-white/70'
                : positive 
                  ? 'border-green-400/30 text-green-300 bg-green-400/5' 
                  : 'border-red-400/30 text-red-300 bg-red-400/5'
            }`}
            whileHover={{ 
              scale: 1.05,
              backgroundColor: (edgePct ?? 0) === 0 
                ? 'rgba(255,255,255,0.04)'
                : positive 
                  ? "rgba(34, 197, 94, 0.1)" 
                  : "rgba(239, 68, 68, 0.1)"
            }}
            transition={{ type: "spring", stiffness: 400, damping: 25 }}
          >
            <div className="text-[10px] font-medium opacity-70 mb-1">Play</div>
            <div>{(edgePct ?? 0) === 0 ? '—' : positive ? 'OVER' : 'UNDER'}</div>
          </motion.div>
        </div>
      </div>

      {/* Click indicator */}
      <motion.div 
        className="absolute bottom-2 right-2 w-6 h-6 rounded-full glass-subtle flex items-center justify-center opacity-0 group-hover:opacity-100"
        whileHover={{ scale: 1.1 }}
        transition={{ duration: 0.2 }}
      >
        <svg className="w-3 h-3 text-white/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
      </motion.div>
    </HoverCard>
  );
}
