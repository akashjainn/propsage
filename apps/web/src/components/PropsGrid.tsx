...
// Memoized PropCard component to prevent unnecessary re-renders
const PropCardComponent = React.memo(function PropCardComponent({ 
  prop, 
  isSelected, 
  onPropClick 
}: { 
  ...
  const formatPropType = React.useCallback((propType: string) => {
    return propType
      .split('_')
...
const PropsGrid: React.FC<PropsGridProps> = React.memo(function PropsGrid({ props, onPropClick, selectedPropId }) {
  if (props.length === 0) {
    return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {props.map((prop) => (
        <PropCardComponent
          ...
          prop={prop}
          isSelected={selectedPropId === `${prop.playerId}-${prop.propType}`}
