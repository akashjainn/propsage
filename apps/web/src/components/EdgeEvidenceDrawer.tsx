...
export default function EdgeEvidenceDrawer({
  ...
  const onTab = (t: 'clips' | 'stats') => {
    setTab(t);
    Sentry.addBreadcrumb({ category: 'ui', level: 'info', message: 'drawer_tab', data: { tab: t } });
  };
  ...
  <div data-testid="evidence-drawer" className="absolute right-0 top-0 h-full w-full sm:w-[520px] bg-background shadow-xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b border-border/60 space-y-2">
          ...
          <div className="flex gap-2 text-sm">
            <button
              className={`px-3 py-1 rounded-md border transition ${tab==='clips' ? 'bg-white/10 border-white/20' : 'border-transparent hover:bg-white/5'}`}
              onClick={() => onTab('clips')}
            >Clips</button>
            <button
              className={`px-3 py-1 rounded-md border transition ${tab==='stats' ? 'bg-white/10 border-white/20' : 'border-transparent hover:bg-white/5'}`}
              onClick={() => onTab('stats')}
            >Stats</button>
          </div>
        </div>
