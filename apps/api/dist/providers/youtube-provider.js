...
import { buildSearchQuery, calculateRelevance } from "../types/social-clips";
...
export class YouTubeProvider {
    ...
    async getClipById(videoId) {
        const cached = videoCache.get(videoId);
        if (cached)
            return cached;
        try {
            const response = await fetch(`${YOUTUBE_BASE_URL}/videos?part=snippet,statistics,contentDetails&id=${videoId}&key=${YOUTUBE_API_KEY}`);
            if (!response.ok)
    ...
    async searchYouTube(searchQuery, maxResults = 8) {
        ...
        const detailsResponse = await fetch(`${YOUTUBE_BASE_URL}/videos?part=snippet,statistics,contentDetails&id=${videoIds}&key=${YOUTUBE_API_KEY}`);
        let detailedVideos = [];
    ...
    convertSearchResultToClip(item) {
        return {
    ...
    async getChannelInfo(channelId) {
        try {
            const response = await fetch(`${YOUTUBE_BASE_URL}/channels?part=snippet,statistics&id=${channelId}&key=${YOUTUBE_API_KEY}`);
            if (response.ok) {
                const data = await response.json();
