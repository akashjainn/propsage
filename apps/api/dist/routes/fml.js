...
function calculateFairLine(books) {
    if (books.length === 0)
        return { fairLine: 0, confidence: 0 };
    let totalWeight = 0;
    let weightedSum = 0;
...
function findEdges(books, fairLine) {
    const edges = [];
    for (const book of books) {
        const { overProb, underProb } = devigOdds(book.over_price, book.under_price);
        // Check over edge
...
router.post('/analyze', async (req, res) => {
    try {
        ...
        // Check cache first
        const cacheKey = `${player_id}-${market}-${event_id}-${books.map((b) => `${b.book}:${b.line}`).join(',')}`;
...
router.get('/edges', async (req, res) => {
    try {
        ...
        const minEdgeThreshold = parseFloat(min_edge);
