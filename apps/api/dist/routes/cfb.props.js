...
r.get("/", async (req, res) => {
    try {
        ...
        if (fs.existsSync(demoPath)) {
            ...
            let filtered = rows;
            if (playerId)
                filtered = filtered.filter(p => p.playerId === playerId);
...
async function buildTeamMarketFallback(playerId) {
    ...
    try {
        ...
        if (!resp.ok)
            throw new Error(`team markets status ${resp.status}`);
        ...
        for (const g of games) {
            ...
            for (const b of g.bookmakers || []) {
                ...
                if (spread) {
                    const outcome = (spread.outcomes || []).find((o) => o.name === team);
                ...
                if (total) {
                    const outcome = (total.outcomes || []).find((o) => o.name === 'Over');
                    if (outcome && typeof outcome.point === 'number') {
                        props.push({
...
function friendlyPlayerName(pid) {
    if (pid === 'cfb_gunner_stockton')
        return 'Gunner Stockton';
