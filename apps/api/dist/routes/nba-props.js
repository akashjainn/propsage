...
const propsCache = new LRUCache({ max: 1000, ttl: 1000 * 60 * 15 });
...
function generatePropId(playerId, stat, book) {
    return `prop_${playerId}_${stat.toLowerCase()}_${book.toLowerCase()}`;
}
function calculateEdgePercent(marketLine, fairLine) {
    if (marketLine === 0)
        return 0;
...
router.get('/props/:propId', async (req, res) => {
    try {
        ...
        const prop = MOCK_PROPS.find(p => p.playerId === playerId &&
            p.stat === stat &&
