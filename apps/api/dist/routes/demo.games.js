...
const gamesCache = new LRUCache({ max: 50, ttl: 1000 * 60 * 5 }); // 5min
const playersCache = new LRUCache({ max: 200, ttl: 1000 * 60 * 10 }); // 10min
...
r.get("/", async (req, res) => {
    try {
        await demoDelay();
        const requestedDate = req.query.date || "2025-09-27";
        ...
        let games = gamesData.games.filter((game) => game.date === requestedDate);
        // Sort by kickoff time
        games = games.sort((a, b) => a.kickoffET.localeCompare(b.kickoffET));
