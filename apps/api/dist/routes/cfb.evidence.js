...
r.get("/prop/:propId", async (req, res) => {
    try {
        ...
        const evidence = await evidenceService.buildPropEvidence(propId, `cfb_${propData.playerName.toLowerCase().replace(/\s+/g, '_')}`, propData.playerName, propData.stat);
        if (!evidence) {
            return res.json({
                propId,
                momentPacks: [],
...
r.get("/moments/:playerId/:propType", async (req, res) => {
    try {
        ...
        const momentPack = await evidenceService.buildMomentPack(playerId, playerName, propType, gameId);
        if (!momentPack) {
            return res.json({
                playerId,
                propType,
...
r.post("/search", async (req, res) => {
    try {
        ...
        const moments = await evidenceService.searchFreefromMoments(query, playerName, gameId, limit);
        res.json({
            query,
            moments,
...
r.post("/index", async (req, res) => {
    try {
        ...
        const videoId = await evidenceService.indexVideoClip(gameId, s3Url, title, teams || [], players || []);
        if (!videoId) {
...
r.get("/demo", async (req, res) => {
    try {
        ...
        const demoMomentPack = await evidenceService.buildMomentPack('cfb_carson_beck', 'Carson Beck', 'PASS_YDS');
        // Create demo evidence for a sample prop
        const demoEvidence = await evidenceService.buildPropEvidence('prop_cfb_beck_pass_yds_dk', 'cfb_carson_beck', 'Carson Beck', 'PASS_YDS');
        res.json({
