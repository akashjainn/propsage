import { videoIntelligenceService } from './twelvelabs.js';
/**
...
export class VideoIntelligentFMLService {
     ...
     */
    async enhanceMarketWithVideo(market) {
     ...
     */
    async gatherVideoSignals(market) {
        const playerQueries = this.buildPlayerQueries(market.player_name, market.market);
        // Search for video signals
        const allSignals = await videoIntelligenceService.searchVideoIntelligence(playerQueries, this.getLeagueFromSport(market.sport));
     ...
     */
    buildPlayerQueries(playerName, market) {
        const baseQueries = [
        ...
        if (market.includes('points') || market.includes('pts')) {
            baseQueries.push(`${playerName} shooting form accuracy`, `${playerName} offensive rhythm confidence`, `${playerName} hand wrist finger injury`);
        }
        if (market.includes('rebounds') || market.includes('reb')) {
            baseQueries.push(`${playerName} back knee injury`, `${playerName} positioning rebounding`, `${playerName} starting lineup rotation`);
        }
        if (market.includes('assists') || market.includes('ast')) {
            baseQueries.push(`${playerName} court vision passing`, `${playerName} shoulder elbow injury`, `${playerName} point guard role change`);
        }
        if (market.includes('threes') || market.includes('3pt')) {
            baseQueries.push(`${playerName} three point shooting`, `${playerName} shooting form mechanics`, `${playerName} confidence rhythm`);
        }
        return baseQueries;
    }
    /**
     ...
     */
    filterAndRankSignals(signals, playerId) {
        const now = new Date();
        return signals
            .filter(signal => {
            // Filter by confidence threshold
            if (signal.confidence < this.videoConfig.min_confidence)
                return false;
     ...
     */
    calculateVideoAdjustments(signals, market) {
        let injuryRisk = 0;
        let minutesRisk = 0;
        let weatherImpact = 0;
        let roleChange = 0;
     ...
     */
    enhancePlayerFeatures(baseFeatures, adjustments) {
        ...
        return {
            ...
            weather_impact: Math.min(1.0, (baseFeatures.weather_impact || 0) + (adjustments.weather_impact * adjustmentWeight * 0.15)),
            // Increase injury probability based on video signals
            injury_probability: Math.min(1.0, (baseFeatures.injury_probability || 0) + (adjustments.injury_risk * adjustmentWeight * 0.5)),
            // Increase minutes restriction probability
            minutes_restriction: Math.min(1.0, (baseFeatures.minutes_restriction || 0) + (adjustments.minutes_risk * adjustmentWeight * 0.6)),
