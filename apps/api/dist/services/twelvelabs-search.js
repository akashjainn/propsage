...
const playerVideoCache = new LRUCache({
    max: 200,
...
export class TwelveLabsSearchService {
     ...
     */
    buildSearchQueries(player, stat, market) {
        const queries = [];
     ...
     */
    async getRelevantVideoIds(player, gameId) {
        // Check cache first
     ...
     */
    async convertMomentsToClips(moments) {
    ...
    processMockData(mockData, params) {
        ...
        for (const [playerId, playerData] of Object.entries(mockData.players)) {
            ...
            for (const video of pData.videos || []) {
                ...
                if (stat && !video.tags?.some((tag) => tag.toLowerCase().includes(stat.toLowerCase()) ||
