...
import { PROP_INTENT_LIBRARY, buildMomentQuery } from "../types/twelve-labs.js";
// Cache for moment packs (1 hour - updated after games)
const momentPackCache = new LRUCache({
    max: 500,
    ttl: 1000 * 60 * 60
});
...
export class EvidenceService {
     ...
     */
    async buildMomentPack(playerId, playerName, propType, gameId) {
        ...
        try {
            ...
            const videoIds = videos.map(v => v.tlVideoId).filter(Boolean);
     ...
     */
    async getMomentPack(playerId, propType, gameId) {
        const cacheKey = `momentpack_${playerId}_${propType}_${gameId || 'all'}`;
     ...
     */
    async buildPropEvidence(propId, playerId, playerName, propType) {
     ...
     */
    async searchFreefromMoments(query, playerName, gameId, limit = 6) {
        if (!isVideoIntelligenceAvailable()) {
            return [];
        }
        try {
     ...
     */
    async indexVideoClip(gameId, s3Url, title, teams, players) {
        if (!isVideoIntelligenceAvailable()) {
     ...
     */
    async batchBuildMomentPacks(players) {
        const results = new Map();
        for (const player of players) {
            const playerPacks = [];
            for (const propType of player.propTypes) {
                const pack = await this.buildMomentPack(player.id, player.name, propType, player.gameId);
                if (pack) {
    ...
    getRelevantVideos(playerName, gameId) {
        return MOCK_VIDEOS.filter(video => {
            if (gameId && video.gameId !== gameId)
                return false;
            if (playerName && !video.players.some(p => p.toLowerCase().includes(playerName.toLowerCase()) ||
                playerName.toLowerCase().includes(p.toLowerCase())))
                return false;
            return video.status === 'ready';
        });
    }
    calculateAverageConfidence(moments) {
        if (moments.length === 0)
            return 0;
    ...
    calculateEvidenceFeatures(momentPacks, propType) {
        if (momentPacks.length === 0)
            return [];
        const allMoments = momentPacks.flatMap(pack => pack.moments);
        const features = [];
        // Feature 1: Explosive Play Rate
        const explosiveMoments = allMoments.filter(m => m.label.toLowerCase().includes('explosive') ||
        ...
        if (propType.includes('TDS')) {
            const redZoneMoments = allMoments.filter(m => m.label.toLowerCase().includes('red zone') ||
        ...
        if (propType.includes('PASS') || propType === 'SACKS' || propType === 'INTERCEPTIONS') {
            const pressureMoments = allMoments.filter(m => m.label.toLowerCase().includes('pressure') ||
    ...
    buildEvidenceSummary(momentPacks, features, propType) {
        const allMoments = momentPacks.flatMap(pack => pack.moments);
        ...
        if (propType.includes('RUSH')) {
            const breakawayMoments = allMoments.filter(m => m.label.includes('broken tackle') || m.label.includes('explosive'));
    ...
    expandQuery(query) {
        ...
        for (const [key, alternatives] of Object.entries(synonyms)) {
            if (query.toLowerCase().includes(key)) {
                ...
                expandedQuery = expandedQuery.replace(new RegExp(key, 'gi'), `(${key} ${alternativeClause})`);
...
export function getMockVideoForDemo() {
    return MOCK_VIDEOS;
}
