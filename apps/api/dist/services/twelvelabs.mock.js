...
class MockTwelveLabsService {
     ...
     */
    async search(query, playerId, tags) {
     ...
     */
    async getIndexStatus(indexId = "idx_cfb_2025_highlights") {
        const data = this.loadMockData();
     ...
     */
    async indexVideo(videoUrl, metadata) {
    ...
    calculateQueryMatch(query, video) {
        const queryLower = query.toLowerCase();
        const titleMatch = video.title.toLowerCase().includes(queryLower) ? 0.8 : 0;
        const tagMatch = video.tags.some((tag) => queryLower.includes(tag)) ? 0.6 : 0;
        const descMatch = video.clips?.some((clip) => clip.description.toLowerCase().includes(queryLower)) ? 0.4 : 0;
