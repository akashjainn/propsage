...
import { getEvidence, EvidenceSnippet } from './demoCache.js'

// LRU cache with TTL and size bound
interface CacheEntry { ts: number; data: EvidenceSnippet[] }
const CACHE_TTL_MS = 5 * 60 * 1000
const CACHE_MAX = 100
const cache = new Map<string, CacheEntry>()

// In-flight dedupe map
const inflight = new Map<string, Promise<EvidenceSnippet[]>>()
...
function setCache(key: string, data: EvidenceSnippet[]) {
...
function getCache(key: string): EvidenceSnippet[] | null {
...
export async function searchEvidence(playerId: string, playerName: string, market: string = 'PTS'): Promise<EvidenceSnippet[]> {
  ...
  const p = (async () => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        const mapped: EvidenceSnippet[] = results.slice(0, 3).map((r: any, i: number) => {
