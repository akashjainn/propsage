...
export interface FairLineInput {
  line: number;
  propId: string;
  seed: string;
  playerForm: number; // 0-1
  opponentRating: number; // 0-1
  pace: number; // 0-1, derived from game context
}

export interface FairLineOutput {
  fairLine: number;
  edgePct: number;
  confidence: number;
  rationale: string[];
...
function mulberry32(seed: number) {
  return function() {
    ...
    let t = seed ^ seed >>> 16; t = Math.imul(t, 0x21f494c7);
    t = t ^ t >>> 13; t = Math.imul(t, 0x735a2d97);
...
export function calculateFairLine(input: FairLineInput): FairLineOutput {
...
export function generateWhyCard(
  input: FairLineInput, 
  result: FairLineOutput,
  playerName: string,
  propType: string,
  stats: any
): {
