...
const momentCache = new LRUCache<string, VideoMoment[]>({ max: 100, ttl: 1000*60*10 }); // 10min
const videoCache = new LRUCache<string, IndexedVideo[]>({ max: 10, ttl: 1000*60*60 }); // 1h

// Mock TwelveLabs integration until we have real indexed videos
const MOCK_INDEXED_VIDEOS: IndexedVideo[] = [
  {
...
function generateMockMoments(query: string, videoIds?: string[]): VideoMoment[] {
...
function getRelevantClipTitle(query: string): string {
  ...
  try {
    ...
    if (team) {
      videos = videos.filter(v => 
        (v.metadata?.team || '').toLowerCase().includes(team) ||
  ...
  try {
    ...
    const uniqueMoments = allMoments
      .filter((moment, index, arr) => 
        arr.findIndex(m => m.videoId === moment.videoId && m.start === moment.start) === index
      )
...
function generatePropQueries(playerName: string, stat: string, team?: string): string[] {
  ...
  switch (stat.toUpperCase()) {
    ...
    case 'PASSING_YARDS':
      ...
      if (team) queries.push(`${team} passing offense ${lastName}`);
      break;
    ...
    case 'RUSHING_YARDS':
      ...
      if (team) queries.push(`${team} rushing attack ${lastName}`);
      break;
    ...
    case 'RECEIVING_YARDS':
      ...
      if (team) queries.push(`${team} passing game ${lastName}`);
      break;
    ...
    default:
      ...
      if (team) queries.push(`${team} ${lastName}`);
  }
