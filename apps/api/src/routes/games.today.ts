...
// We will highlight the existing demo primary game (Haynes King) by injecting a flag.

interface TodayGame extends NormalizedGame {
  demoFeatured?: boolean;
}

function loadDemoGame(): Partial<TodayGame> | null {
  try {
    ...
    if (fs.existsSync(demoPath)) {
      ...
      if (Array.isArray(raw)) {
        const g = raw.find((r: any) =>
          (r.home?.team || '').toLowerCase().includes('georgia tech') ||
...
router.get('/games/today', async (req, res) => {
  ...
  let enriched: TodayGame[] = games.map(g => ({ ...g }));
  if (demoMeta) {
    // Try to match an existing schedule game with same home/away team, else push synthetic
    const idx = enriched.findIndex(g =>
      g.home.team.toLowerCase().includes('georgia tech') ||
    ...
    if (idx >= 0) {
      ...
      enriched.push({
        ...
        status: (demoMeta as any).status || 'SCHEDULED',
        home: (demoMeta as any).home || { team: 'Georgia Tech', score: null },
        away: (demoMeta as any).away || { team: 'Opponent', score: null },
  ...
  enriched.sort((a,b) => {
    if (a.demoFeatured && !b.demoFeatured) return -1;
    if (b.demoFeatured && !a.demoFeatured) return 1;
