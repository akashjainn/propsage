...
const insightsCache = new LRUCache<string, any>({ max: 200, ttl: 1000 * 60 * 5 }); // 5min
...
r.get("/:propId/insights", async (req, res) => {
  try {
    ...
    Object.keys(propsData.props).forEach(gId => {
      const prop = propsData.props[gId].find((p: any) => p.id === propId);
      if (prop) {
        foundProp = prop;
        gameId = gId;
      }
    ...
    const whyCard = generateWhyCard(
      fairLineInput,
      fairLineResult,
      playerData?.name || foundProp.playerId,
      foundProp.type,
      playerData?.stats || {}
    );
...
function findPlayerData(playersData: any, playerId: string): any {
  if (!playersData || !playersData.players) return null;
